...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const nameOfTeacher =
        user?.name || teacherName?.trim() || publishedBy?.trim() || "Teacher";
  ...
  async (req, res) => {
    try {
      ...
      const isAuthor = Boolean(
        (user.email && existingNotice.authorEmail === user.email) || false,
      );
      ...
      const { title, detail, category, isPinned, teacherName } = req.body;
